...
export default function FormatTab({ apiKey, aiModel, hasKey }) {
    ...
    const handleSet = useCallback((k, v) => {
        if (k === 'author') setAuthor(v)
        else if (k === 'title') setTitle(v)
    }, [])
    ...
    return (
        <TabPanel>
            ...
            <div className="tool-fields">
                <MemoizedFieldInput label="Author Name" fieldKey="author" value={author} onChange={handleSet} placeholder="Jane Smith" required />
                <MemoizedFieldInput label="Manuscript Title" fieldKey="title" value={title} onChange={handleSet} placeholder="The Lost Hours" required />
