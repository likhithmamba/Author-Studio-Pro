/**
 * PlotMention — TipTap Mention extension for #plot tokens.
 * Typing #PlotPoint shows a dropdown of known plot nodes; selecting one
 * creates or references a plot node in the Zustand store.
 */
...
export const PlotMention = Mention.extend({
  name: 'plotMention',
}).configure({
  HTMLAttributes: {
    class: 'plot-mention',
  ...
  suggestion: {
    ...
    command: ({ editor, range, props }) => {
      ...
      editor
        ...
        .deleteRange(range)
        .insertContent([
          { type: 'plotMention', attrs: { id: nodeId, label: props.label || props.id } },
          { type: 'text', text: ' ' },
        ])
        .run()
    },
